...
public class ImportLegacyLogItemAction(
...
) : ApiAction<UnverifyContext>(serviceProvider)
{
    public override async Task<ApiResult> ProcessAsync()
    {
        ...
        switch (entity.OperationType)
        {
            ...
            case UnverifyOperationType.Recovery:
                entity.RemoveOperation = ParseRemoveOperation(dataField);
                break;
            case UnverifyOperationType.Update:
                entity.UpdateOperation = ParseUpdateOperation(dataField);
                break;
        }
    ...
    private static UnverifyLogRemoveOperation ParseRemoveOperation(JsonObject jsonData)
    {
        return new UnverifyLogRemoveOperation
        {
            Channels = [
                .. (jsonData["ReturnedOverwrites"]?.AsArray() ?? []).Select(node => new UnverifyLogRemoveChannel
                {
                    ChannelId = ReadRequiredField<ulong>(node!, "ChannelId"),
                    AllowValue = ReadRequiredField<ulong>(node!, "AllowValue"),
                    DenyValue = ReadRequiredField<ulong>(node!, "DenyValue")
            ...
            Force = ReadOptionalField<bool>(jsonData, "Force"),
            IsFromWeb = ReadOptionalField<bool>(jsonData, "FromWeb"),
            Language = ReadOptionalField<string>(jsonData, "Language"),
            Roles = [
                .. (jsonData["ReturnedRoles"]?.AsArray() ?? []).Select(node => new UnverifyLogRemoveRole
                {
                    RoleId = node!.GetValue<ulong>()
                })
            ]
    ...
    private static UnverifyLogUpdateOperation ParseUpdateOperation(JsonObject jsonData)
    {
        var end = ReadRequiredField<DateTime>(jsonData, "End");
        if (end.Kind == DateTimeKind.Unspecified)
            end = end.WithKind(DateTimeKind.Local);
        ...
        return new UnverifyLogUpdateOperation
        {
            NewEndAtUtc = end.ToUniversalTime(),
            NewStartAtUtc = start.ToUniversalTime(),
            Reason = ReadOptionalField<string>(jsonData, "Reason")
        };
    }
